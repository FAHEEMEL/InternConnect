...
const JobListing = () => {
    ...
    useEffect(()=> {
        ...
        const visibilityFilter = job => job.visible

        const newFilteredJobs = jobs.slice().reverse().filter(
            job => matchesLocation(job) && matchesCategory(job) && matchesTitle(job) && matchesSearchLocation(job) && visibilityFilter(job)
        )

        // console.log(newFilteredJobs[0].location)
                ...
                <ul className='space-y-4 text-gray-600'>
                    {
                        JobLocations.map((location, index) => (
                            <li className='flex gap-3 items-center' key={index}>
                                ...
                                className='scale-125' 
                                type="checkbox"
                                onChange={() => handleLocationChanges(location)} 
